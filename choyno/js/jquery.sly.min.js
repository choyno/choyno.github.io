...
f=d[a.horizontal?"outerWidth":"outerHeight"](!0),j=Z(d,a.horizontal?"marginLeft":"marginTop"),d=Z(d,a.horizontal?"marginRight":"marginBottom"),l={size:f,start:p-(!c||a.horizontal?0:j),center:p-Math.round(u/2-f/2),end:p-u+f-(k?0:d)};c||(ea=-(B?Math.round(u/2-f/2):0)+b,p+=b);p+=f;!a.horizontal&&!n&&d&&(j&&0<c)&&(p-=Math.min(j,d));c===t.length-1&&(p+=g);m.push(l)});s[0].style[a.horizontal?"width":"height"]=p+"px";p-=c;d.start=ea;d.end=B?m[m.length-1].center:Math.max(p-u,0);j.activeItem>=m.length?e.activate(m.length-
1):1===m.length&&t.eq(j.activeItem).addClass(a.activeClass)}d.center=Math.round(d.end/2+d.start/2);h.extend(j,ka(void 0));y&&(N=a.dynamicHandle?Math.round(ca*u/p):y[a.horizontal?"outerWidth":"outerHeight"](),a.dynamicHandle&&(N=H(N,a.minHandleSize,ca),y[0].style[a.horizontal?"width":"height"]=N+"px"),q.end=ca-N,M||O());if(!C){var l=d.start,c="",x=0;if(G)h.each(m,function(a,c){if(B||c.start+c.size>l)l=c[B?"center":"start"],v.push(l),l+=u});else for(;l-u<=d.end;)v.push(l),l+=u;if(Y[0]){for(var r=0;r<
v.length;r++)c+=a.pageBuilder(x++);aa=h(c).appendTo(Y.empty())}}F(H(d.dest,d.start,d.end));j.pages=v.length;j.slideeSize=p;j.frameSize=u;j.sbSize=ca;j.handleSize=N;z("load")};e.getPos=function(c){if(c===Q)return d;if(G)return c=ma(c),-1!==c?m[c]:!1;var b=s.find(c).eq(0);return b[0]?(c=a.horizontal?b.offset().left-s.offset().left:b.offset().top-s.offset().top,b=b[a.horizontal?"outerWidth":"outerHeight"](),{start:c,center:c-u/2+b/2,end:c-u+b,size:b}):!1};e.getRel=function(){return j};e.prev=function(){e.activate(j.activeItem-
1)};e.next=function(){e.activate(j.activeItem+1)};e.prevPage=function(){e.activatePage(j.activePage-1)};e.nextPage=function(){e.activatePage(j.activePage+1)};e.slideBy=function(c,a){F(d.dest+c,a)};e.slideTo=function(a,b){F(a,b)};e.toStart=function(a,b){P("start",a,b)};e.toEnd=function(a,b){P("end",a,b)};e.toCenter=function(a,b){P("center",a,b)};e.activate=function(c,d){if(G&&c!==Q){var g=ma(c),f=j.activeItem;j.activeItem=g;t.eq(f).removeClass(a.activeClass);t.eq(g).addClass(a.activeClass);$();g!==
f&&z("active",g);d||(R?e.toCenter(g):va&&(g>=j.lastItem?e.toStart(g):g<=j.firstItem?e.toEnd(g):b.released&&!S&&e.cycle()))}};e.activatePage=function(a){v.length&&(a=H(a,0,v.length-1),F(v[a]),z("activePage",a))};e.cycle=function(c){if(a.cycleBy&&a.cycleInterval&&!("items"===a.cycleBy&&!m[0]||c&&S))S=0,K?K=clearTimeout(K):z("cycleStart"),K=setTimeout(function(){switch(a.cycleBy){case "items":e.activate(j.activeItem>=m.length-1?0:j.activeItem+1);break;case "pages":e.activatePage(j.activePage>=v.length-
1?0:j.activePage+1)}z("cycle")},a.cycleInterval)};e.pause=function(a){a||(S=!0);K&&(K=clearTimeout(K),z("cyclePause"))};e.toggle=function(){e[K?"pause":"cycle"]()};e.set=function(c,b){h.isPlainObject(c)?h.extend(a,c):a.hasOwnProperty(c)&&(a[c]=b)};e.on=function(a,b){if("object"===typeof a)for(var g in a){if(a.hasOwnProperty(g))e.on(g,a[g])}else if(b instanceof Array){g=0;for(var d=b.length;g<d;g++)e.on(a,b[g])}else if("function"===typeof b){l[a]=l[a]||[];g=0;for(d=l[a].length;g<d;g++)if(l[a][g]===
b)return;l[a].push(b)}};e.off=function(a,b){if(l[a])if(b instanceof Array)for(var g=0,d=b.length;g<d;g++)e.off(a,b[g]);else if("function"===typeof b){g=0;for(d=l[a].length;g<d;g++)l[a][g]===b&&l[a].splice(g,1)}else l[a].length=0};e.destroy=function(){ga.add(s).add(wa).add(y).add(E).add(Y).add(U).add(V).add(W).add(X).unbind("."+k);U.add(V).add(W).add(X).removeClass(a.disabledClass);t&&t.eq(j.activeItem).removeClass(a.activeClass);Y.empty();C||(D.unbind("."+k),s.add(y).css(A||(a.horizontal?"left":"top"),
A?"none":0),h.removeData(f,k))};e.init=function(){if(!ua){e.on(ea);if(!C){D.css("overflow","hidden");var b=s.add(y);if(A){var f={};f[A]="translateZ(0)";b.css(f)}else"static"===D.css("position")&&D.css("position","relative"),b.css({position:"absolute"})}!A&&"static"===E.css("position")&&E.css("position","relative");za();G?e.activate(a.startAt):F(a.startAt);if(a.scrollBy)wa.on("DOMMouseScroll."+k+" mousewheel."+k,function(b){if(d.start!==d.end){r(b,1);b=b.originalEvent;var c=0;b.wheelDelta&&(c=0>b.wheelDelta/
120);b.detail&&(c=0>-b.detail/3);G?(b=ma((R?B?j.activeItem:j.centerItem:j.firstItem)+(c?a.scrollBy:-a.scrollBy)),e[R?B?"activate":"toCenter":"toStart"](b)):e.slideBy(c?a.scrollBy:-a.scrollBy)}});if(a.clickBar&&E[0])E.on("click."+k,function(b){1>=b.which&&(r(b),F(da((a.horizontal?b.clientX-E.offset().left:b.clientY-E.offset().top)-N/2)))});a.keyboardNavBy&&ga.bind("keydown."+k,function(b){switch(b.which){case a.horizontal?37:38:r(b);e["pages"===a.keyboardNavBy?"prevPage":"prev"]();break;case a.horizontal?
...
{source:"slidee"},qa);if(a.dragHandle&&y)y.on(xa,{source:"handle"},qa);if(a.cycleBy&&!C){if(a.pauseOnHover)D.on("mouseenter."+k+" mouseleave."+k,function(a){if(!S)e["mouseenter"===a.type?"pause":"cycle"](1)});e[a.startPaused?"pause":"cycle"]()}ua=1;return e}}}function r(f,a){f=f||w.event;f.preventDefault?f.preventDefault():f.returnValue=!1;a&&(f.stopPropagation?f.stopPropagation():f.cancelBubble=!0)}function T(f){return!isNaN(parseFloat(f))&&isFinite(f)}function Z(f,a){return parseInt(f.css(a),10)||
0}function H(f,a,h){return f<a?a:f>h?h:f}for(var ba="sly",k=ba,n=w.cancelAnimationFrame||w.cancelRequestAnimationFrame,fa=w.requestAnimationFrame,A,la,L=window,O=["moz","webkit","o"],da=0,J=0,na=O.length;J<na&&!n;++J)fa=(n=L[O[J]+"CancelAnimationFrame"]||L[O[J]+"CancelRequestAnimationFrame"])&&L[O[J]+"RequestAnimationFrame"];n||(fa=function(f){var a=+new Date,h=Math.max(0,16-(a-da));da=a+h;return L.setTimeout(function(){f(a+h)},h)},n=function(f){clearTimeout(f)});var n=function(f){for(var a=0,h=P.length;a<
h;a++){var k=P[a]?P[a]+f.charAt(0).toUpperCase()+f.slice(1):f;if(oa.style[k]!==Q)return k}},P=["","webkit","moz","ms","o"],oa=document.createElement("div");A=n("transform");la=n("perspective")?"translateZ(0) ":"";w.Sly=$;h.fn[ba]=function(f,a){var n,r;if(!h.isPlainObject(f)){if("string"===typeof f||!1===f)n=!1===f?"destroy":f,r=Array.prototype.slice.call(arguments,1);f={}}return this.each(function(A,w){var x=h.data(w,k);!x&&!n?h.data(w,k,(new $(w,f,a)).init()):x&&n&&x[n]&&x[n].apply(x,r)})};h.fn[ba].defaults=
